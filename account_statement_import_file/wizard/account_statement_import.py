...
class AccountStatementImport(models.TransientModel):
    ...
    def _import_file(self):
        ...
        statement_ids, ignored_qty = self.import_single_file(file_data, result)
        ...
        self.env["ir.attachment"].create(self._prepare_create_attachment(statement_ids))
        return statement_ids, []

    def import_file_button(self):
        """Process the file chosen in the wizard, create bank statement(s)
        and return an action."""
        statement_ids, notifications = self._import_file()
    ...
    def _prepare_create_attachment(self, statement_ids):
    ...
    def import_single_file(self, file_data, result):
        ...
        i = 0
        statement_ids, ignored_qty = [], False
        for single_statement_data in parsing_data:
            i += 1
            logger.debug(
                "account %d: single_statement_data=%s", i, single_statement_data
            )
            statement_ids_tmp, ignored_qty = self.import_single_statement(single_statement_data, result)
            statement_ids += statement_ids_tmp
    ...
    def import_single_statement(self, single_statement_data, result):
        ...
        statement_ids, ignored_qty = self._create_bank_statements(stmts_vals, result)
    ...
    def _create_bank_statements(self, stmts_vals, raise_no_imported_file=True):
        ...
        for st_vals in stmts_vals:
            ...
            if len(filtered_st_lines) > 0:
                ...
                if not filtered_st_lines[0].get("sequence"):
                    for seq, vals in enumerate(filtered_st_lines, start=1):
                        vals["sequence"] = seq
