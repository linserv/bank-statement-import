...
class TestParserCommon(TransactionCase):
    ...
    def _get_files_diffs(self, inputfile_path, goldenfile_path) -> list:
        ...
        data_path = misc.file_path(inputfile_path)
        with open(data_path, "rb") as inputf:
            res = self.parser.parse(inputf.read())
        # Read the output file, store the expected imported values
        data_path2 = misc.file_path(goldenfile_path)
        with open(data_path2, "r") as goldf:
...
class TestImport(TransactionCase):
    ...
    def test_statement_import(self):
        ...
        testfile = misc.file_path("account_statement_import_camt/test_files/test-camt053")
    ...
    def test_zip_import(self):
        ...
        testfile = misc.file_path("account_statement_import_camt/test_files/test-camt053.zip")
        with open(testfile, "rb") as datafile:
            camt_file = base64.b64encode(datafile.read())
            self.env["account.statement.import"].create(
